...
if uploaded_file is not None:
    ...
    reverse_mapping = {col: {v: k for k, v in mapping.items()} for col, mapping in mapping.items()}
    ...
    if selected_method == "Tính xấp xỉ":
        # Chọn tập thuộc tính
        st.markdown('<label for="tap-thuoc-tinh">Chọn tập thuộc tính:</label>', unsafe_allow_html=True)
        selected_attributes = st.multiselect("Chọn tập thuộc tính", attributes)

        # Tạo từ điển ánh xạ ngược từ số sang chữ (reverse mapping)
        reverse_mapping_decision = {v: k for k, v in mapping[decision_column].items()}

        # Lấy danh sách các giá trị thuộc tính quyết định dưới dạng chữ
        unique_decision_values = [reverse_mapping_decision[val] for val in df[decision_column].dropna().unique()]
        ...
        decision_value = {v: k for k, v in reverse_mapping_decision.items()}[decision_value_label]
    ...
    if st.button("Thực hiện"):
        if selected_method == "Tính xấp xỉ":
            if not selected_attributes:
                ...
                def approximation_lower(df, attributes, decision_value):
                    ...
                    for idx, row in df.iterrows():
                        ...
                        subset = df[(df[attributes] == attribute_values).all(axis=1)][decision_column]
                ...
                # Tính toán xấp xỉ
                lower_count, lower_set = approximation_lower(df, selected_attributes, decision_value)
                upper_count, upper_set = approximation_upper(df, selected_attributes, decision_value)
            ...
            if not selected_attributes:
                ...
                df = df.dropna(subset=selected_attributes + [decision_column])  # Loại bỏ giá trị NaN
                ...
                lower_approximation = approximation_lower(df, selected_attributes, decision_column)
                upper_approximation = approximation_upper(df, selected_attributes, decision_column)

                # Tính hệ số phụ thuộc
                k = lower_approximation / len(df) if len(df) > 0 else 0  # Hệ số phụ thuộc
                st.write(f"Hệ số phụ thuộc (k): {k:.2f}")

                # Tính độ chính xác
                accuracy = lower_approximation / upper_approximation if upper_approximation != 0 else 0
            ...
            attributes = columns[1:-1]  # Chỉ lấy các thuộc tính (bỏ cột đầu tiên và cột quyết định)
            ...
            def generate_classification_rules_with_reverse_mapping(df, reduct, decision_column, reverse_mapping):
                ...
                for _, subset in df.groupby(list(reduct)):
                    ...
                    if len(decision_values) == 1:  # Đảm bảo chỉ có 1 giá trị duy nhất
                        ...
                        decision_label = reverse_mapping[decision_column].get(decision_value, decision_value)
